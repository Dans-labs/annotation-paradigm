...
Step 3: context analysis (yet another idea renewed)
...
Now we are going to link positions in different sources to each other.

Initially we give unrelated position numbers to all words in all sources. No two words will have the same position number.
Based on context analysis we will assign the same number to positions that have equal context.

When we compute contexts, we compare pairs <number, clusterocc>
Two pairs are considered equal if either the numbers are equal or the clusteroccs, or both.

Due to context analysis, contexts become more equal, so in a next round we can identify even more positions.

So the datastructure is

%sources{passage}->{source}->[num] = [code, clusterocc]
...
";
	...
	while (my @row = $sth->fetchrow_array()) {
		...
		$nwords++;
		$sourcelist{$source} = 1;
		$sources{$passage}->{$source}->[$wordnumber-1] = [++$code, $glyphs];
	}
	my $sourcepassages = 0;
...
# the weight counts how many surface word occurrences in the sources are linked to this cluster occurrence
				...
				for my $source (sort keys %$sourceinfo) {
					...
					for my $wordnum (sort keys %$wordinfo) {
						my $word = $sources{$passage}->{$source}->[$wordnum]->[1];
						$collectedwords{$word}++;
					}
				}
