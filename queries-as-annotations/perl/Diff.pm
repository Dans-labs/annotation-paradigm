...
# I have adapted the role of the keyGen code reference.
# It is expected to return reference to an array of keys.
# So you have the opportunity to assign multiple keys to an item.
# The meaning of multiple keys is DISJUNCTIVE: two items are equal
# if and only if they have a key in common.
# With single keys you cannot accomodate relaxed equality definitions,
# of the kind where you compare an item with several alternatives.
...
sub _longestCommonSubsequence
{
    ...
    else
    {
        $compare = sub {
            my $a = shift;
            my $b = shift;
			for my $ka (@{&$keyGen( $a, @_ )}) {
				for my $kb (@{&$keyGen( $b, @_ )}) {
					if ($ka eq $kb) {
						return 1;
					}
				}
