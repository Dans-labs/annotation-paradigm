...
my ($sql_template, $with_monads, $sql_dump, $anchor_file, $listing) = @ARGV;
...
while ($line = <FW>) {
	...
	if ($line =~ m/<w /) {
		...
		my $anchprefix = sprintf "%s_%03d:%03d^", $bk_id_acro{$book_id}, $chapter_num, $verse_num;
		my %localanchormap = ();
		my @localanchors = ();
		my $realword_num = 0;
		for (my $local_word_num = 0; $local_word_num < scalar(@words); $local_word_num++) {
			my $word = $words[$local_word_num];
			my ($w) = $word =~ m/<w id="m([0-9]+)"/;
			my $thisnum;
			my $wordempty = $word =~ m/></;
			if ($wordempty) {
				$thisnum = $realword_num;
			}
			else {
				$thisnum = ++$realword_num;
			}
			my $anch = sprintf "%s%03d", $anchprefix, $thisnum;
			printf A "%s\t%d\n", $anch, $w;
			$localanchormap{$w} = $anch;
			if (!$wordempty) {
				push @localanchors, $anch;
			}
		}
		my ($rest, $pre, $thisword, $newrest);
		$rest = $line;
		my $resultline = '';
		while ($rest ne '') {
			($pre, $thisword, $newrest) = $rest =~ m/^(.*?)(<w[^>]*>[^<]*<\/w>)(.*)$/s;
			if (!defined $pre) {
				$pre = $rest;
				$thisword = '';
				$newrest = '';
			}
			my ($monadnum, $innerword) = $thisword =~ m/<w id="m([0-9]+)"[^>]*>([^<]*)</;
			my $anch = $localanchormap{$monadnum};
			$thisword =~ s/id="m[0-9]+"/id="$anch"/;
			$resultline .= $pre . $thisword;
			if (length $innerword) {
				for my $d (dediac($innerword)) {
					#printf L "%s\n", $d;
					printf L "%s\t%s\n", $anch, $d;
				}
			}
