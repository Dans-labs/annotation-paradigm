...
# 2012-03-29 modified by Dirk Roorda
# added code parameter isEqua to traverse_balanced, traverse_sequences and to
# _longestCommonSubsequence
#
# This increases the generality of the objects on which the algorithm can be run.
# Suppose you want to compare two list a and list b and return true if and only if they have
# an element in common. 
# This cannot be done by generating keys for list a and for list b and then eq-ing them.
...
sub _longestCommonSubsequence
{
	...
	if ( defined($isEqua) )	   # user supplied equality method, if supplied
	{
		$compare = $isEqua;
	}
...
sub traverse_sequences
{
    ...
    my $matchVector = _longestCommonSubsequence( $a, $b, 0, $keyGen, $isEqua, @_ );
...
C<LCS> may be passed an optional third and fourth parameter; these are CODE
references to a key generation function and a comparison method.
See L</KEY GENERATION FUNCTIONS>.

    @lcs    = LCS( \@seq1, \@seq2, \&keyGen, \&isEqua, @args );
    $lcsref = LCS( \@seq1, \@seq2, \&keyGen, \&isEqua, @args );
...
=head2 C<new>
    ...
    $diff = Diffs->new( \@seq1, \@seq2 );
    $diff = Diffs->new( \@seq1, \@seq2, \%opts );
...
C<sdiff> may be passed an optional third and fourth parameter; this are CODE
references to a key generation function and a comparison method.  See L</KEY GENERATION
FUNCTIONS>.
...
C<traverse_sequences> may be passed an optional fourth and fifth parameter; these
are CODE references to a key generation function and a comparison method.
See L</KEY GENERATION FUNCTIONS>.
...
Most of the functions accept an optional extra parameter or two.  There are
CODE references to a key generating (hashing) function and a comparison method.
The key generating function should return
...
If this generality is not enough, you can also override the comparison method:
supply a code ref to a subroutine that takes two arguments, and returns a 0 or 1.
When this method returns 1, the algorithm considers the two objects to be equal.

For instance (key generation), consider this example:
...
equivalent, you would have to pass in a key generation function:
    ...
    my $array1 = [ $person1, $person2, $person4 ];
    my $array2 = [ $person1, $person3, $person4->clone(), $person5 ];
    Diff::diff( $array1, $array2, \&Person::hash );
...
list for announcements related to diff and Diff, send an
